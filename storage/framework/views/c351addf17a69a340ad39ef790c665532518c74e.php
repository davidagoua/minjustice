...
<?php endif; ?>
<?php $component->withAttributes(['dark-mode' => config('filament.dark_mode'),'attributes' => \Filament\Support\prepare_inherited_attributes($attributes)->merge($action->getExtraAttributes()),'form' => $action->getForm(),'tag' => $action->getUrl() ? 'a' : 'button','wire:click' => $wireClickAction,'href' => $action->isEnabled() ? $action->getUrl() : null,'target' => $action->shouldOpenUrlInNewTab() ? '_blank' : null,'type' => $action->canSubmitForm() ? 'submit' : 'button','color' => $action->getColor(),'key-bindings' => $action->getKeybindings(),'tooltip' => $action->getTooltip(),'disabled' => $action->isDisabled(),'icon' => $icon ?? $action->getIcon(),'size' => $action->getSize(),'dusk' => 'filament.admin.action.'.e($action->getName()).'']); ?>
    <?php echo e($slot); ?>
...
<?php endif; ?>
<?php /**PATH C:\Users\user\PhpstormProjects\minijust\vendor\filament\filament\src\/../resources/views/components/pages/actions/action.blade.php ENDPATH**/ ?>